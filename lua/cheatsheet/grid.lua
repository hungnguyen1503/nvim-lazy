...
return function()
    ...
    local win = require("cheatsheet.api").getLargestWin()
    ...
    -- prevent not enough space
    if win_width < column_width then return nil end
            ...
            if type(modeMappings) == "table" then
                for keystroke, mapping_info in pairs(modeMappings) do
                    if mapping_info[2] then
                        local whitespace_len = column_width - 4 -
                            vim.fn.strdisplaywidth(prettify_Str(keystroke) .. mapping_info[2])
                        local pretty_mapping = mapping_info[2] ..
                            string.rep(" ", whitespace_len) .. prettify_Str(keystroke)
        ...
        for column_i, _ in ipairs(columns) do
            ...
            if columns[column_i][i] then
                ...
                if vim.tbl_contains(card_headings, columns[column_i][i]) then
                    ...
                    local random_hl_group = highlight_groups[math.random(1, #highlight_groups)]
